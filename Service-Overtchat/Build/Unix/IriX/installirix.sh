...
# Depend du fichier overtchat.conf
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
export OVERTCHAT_CONF="$SCRIPT_DIR/../../Conf/overtchat.conf"

if [[ -v OVERTCHAT_CONF ]]; then
    if [[ -f "$OVERTCHAT_CONF" ]]; then
        source "$OVERTCHAT_CONF"
    fi
fi
