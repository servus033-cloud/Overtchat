...
if [ $# -lt 1 ]; then
  printf "%s\n" "Usage: $0 <path-to-extracted-Service-Overtchat-dir-or-archive>"
  exit 1
fi
...
printf "%s\n" "Installation depuis $SRC_DIR vers $TARGET"
...
printf "%s\n" "Installation terminée dans $TARGET"
printf "%s\n" "Lance le panel : $TARGET/bin/setup-overtchat"
