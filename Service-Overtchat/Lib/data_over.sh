#!/bin/bash
                                        # ───────────────────────────────────── #
                                        #   Création du fichier overtchat.conf
                                        # ───────────────────────────────────── #

create_over_conf() {
    if [[ "$(find "$HOME/Service-Overtchat/Conf" -type f -name "overtchat.conf" -print -quit 2>/dev/null)" ]]; then
        printf "%s\n" "Fichier overtchat.conf déjà existant [ actif ]"; return 0
    fi

    printf "%s\n" "Création du fichier overtchat.conf [ en cours... ]"
    log "Création du fichier overtchat.conf (Code erreur : 048) [en cours]"
    sleep 2
...
# Gestion Git
web["git_repo"]="https://github.com/servus033-cloud/Overtchat"
web["git_branch"]="main"
web["git_url"]="${web[git_repo]}.git"
web["git_data"]="$HOME/Overtchat"
...
shell["build_mkpass"]="${folders[dir_lib]}/mkpass.sh"           # Générateur password crypté
shell["build_mails"]="${folders[dir_lib]}/sendmails.sh"         # Générateur email
...
GLOBAL
    ...
    log "Création du fichier overtchat.conf (Code erreur : 048) [validé]"
}